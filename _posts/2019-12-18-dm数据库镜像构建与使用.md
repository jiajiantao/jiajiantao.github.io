# <center>dm7数据库镜像的构建与使用</center>

* 下载dm7开发版rh7-64安装包，https://pan.baidu.com/s/1VOp3IUYA1ZVjrZoNLrkhyw 提取码: 96cr，从安装包中得到 DMInstall.bin
*  在当前目录下新建文件config.xml，文件内容为
```xml
<?xml version="1.0"?>
<DATABASE>
...
<!--安装路径,不允许为空。 -->
<INSTALL_PATH>/opt/dm</INSTALL_PATH>
<!--是否初始化库,取值 Y/N、y/n,不允许为空。 -->
<INIT_DB>y</INIT_DB>
<!--数据库实例参数 -->
<DB_PARAMS>
<!--初始数据库存放的路径,不允许为空 -->
<PATH>/opt/dm/data</PATH>
...
```
* 将DMInstall.bin文件上传到当前目录下
* 在当前目录下新建文件Dockerfile，文件内容如下
```
FROM centos:7
...
WORKDIR /opt/dm
RUN  mkdir -p /opt/dm; \
     chmod +x /opt/DMInstall.bin; \
    /opt/DMInstall.bin -q /tmp/config.xml;\
    rm -rf /opt/DMInstall.bin /tmp/config.xml
...
* 在当前目录下执行命令构建镜像
```
docker build -t registry.cn-hangzhou.aliyuncs.com/jiajiantao/dm:7.6.0.197 .
```
* 执行命令启动dm7数据库
```
docker run -d --name dmserver -p 5236:5236 registry.cn-hangzhou.aliyuncs.com/jiajiantao/dm:7.6.0.197
```
* 使用dbeaver连接数据库，添加dm7驱动，jdbc url模板设置为:jdbc:dm://{host}:{port}，host为主机地址，port为容器映射到宿主机的端口，用户名/密码：sysdba/123456789
